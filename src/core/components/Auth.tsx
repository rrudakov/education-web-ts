...
import { getAuthOpen, getSignInUsername, getSignInPassword } from '../selector';
import { CLOSE_AUTH, UPDATE_SIGN_IN_USERNAME, UPDATE_SIGN_IN_PASSWORD } from '../types';
...
export const SignInDialog: React.FC = () => {
  ...
  const closeAuth = useCallback(() => dispatch({ type: CLOSE_AUTH }), [dispatch]);
  ...
  const sendSignInRequest = useCallback(() => dispatch(thunkLogin()), [dispatch]);

  return (
    <Dialog open={authOpen} onClose={closeAuth} aria-labelledby="signin-dialog-title">
      ...
      <DialogActions>
        ...
        <Button onClick={sendSignInRequest}>
          Sign in
        </Button>
      </DialogActions>
    </Dialog>
